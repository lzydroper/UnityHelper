...
def get_locked_model_id() -> str:
    return (os.environ.get('OPENWEBUI_LOCKED_MODEL_ID') or os.environ.get('DIFY_MODEL_ID') or '').strip()


def get_locked_model_payload(model_id: str, url_idx: int = 0) -> dict:
    return {
        'id': model_id,
        'name': model_id,
        'object': 'model',
...
async def get_all_models_responses(request: Request, user: UserModel) -> list:
    ...
    request_tasks = []
    locked_model_id = get_locked_model_id()
            ...
            request_tasks.append(
                asyncio.ensure_future(
                    asyncio.sleep(
                        ...
                        {
                            'object': 'list',
                            'data': [get_locked_model_payload(locked_model_id, idx)],
                        },
                    )
                )
            )
...
async def get_models(request: Request, url_idx: Optional[int] = None, user=Depends(get_verified_user)):
    ...
    if locked_model_id:
        models['data'] = [
            model
            for model in models.get('data', [])
            if (model.get('id') or model.get('name')) == locked_model_id
        ] or [get_locked_model_payload(locked_model_id, url_idx or 0)]
...
async def verify_connection(
    ...
    if get_locked_model_id():
        raise HTTPException(status_code=403, detail='OpenAI-compatible connection is locked by local configuration')
...
async def generate_chat_completion(
    ...
    locked_model_id = get_locked_model_id()
    if locked_model_id:
        payload['model'] = locked_model_id
        form_data['model'] = locked_model_id
    ...
    if model_info:
        ...
        if not locked_model_id or model_id != locked_model_id:
